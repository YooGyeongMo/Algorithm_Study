...
입력은 여러 개의 테스트 케이스로 주어지며, 첫째 줄에 테스트 케이스의 개수가 주어진다. 각 테스트 케이스는 다음과 같은 형식이다.
	...
	첫째 줄에 지워야 하는 파일의 개수 N_(1)이 주어진다. (1 ≤ N_(1) ≤ 1000)
	이어서 N_(1)개의 줄에 지워야 하는 파일명이 줄마다 하나씩 주어진다.
	이어서 지우면 안 되는 파일의 개수 N_(2)가 주어진다. (0 ≤ N_(2) ≤ 1000)
	이어서 N_(2)개의 줄에 지우면 안 되는 파일명이 줄마다 하나씩 주어진다.
